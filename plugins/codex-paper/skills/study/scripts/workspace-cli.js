...
function usage() {
  console.error('Usage: workspace-cli.js <list|inspect|write|tags|abandon> ...')
}
...
async function main() {
  ...
  if (command === 'list') {
    ...
    result = await writeWorkspaceAuthoring(workspace, relativePath, data, absent ? { expectAbsent: true } : { expectedSha256 }, { lockTimeoutMs })
...
main().catch((error) => {
  const code = String(error?.code || 'WORKSPACE_OPERATION_FAILED')
