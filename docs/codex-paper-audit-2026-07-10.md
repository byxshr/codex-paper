...
- **当前状态：M3 进行中；P1-3a、P1-4 均已完成 Review、推送和远端 CI；P1-2a 已完成独立 Review、尚未推送，远端 CI 通过后进入 P1-2b。**
- **主线基础链必须按顺序执行：**
...
- 当前阶段：`M3`“必要 P1 工程与恢复能力”；M0、M1、M2 均已正式关闭。P1-3a 与 P1-4 均已完成独立 Review、阶段推送和远端 CI；P1-2a 已完成独立 Review并保持未推送，下一开发项为 `P1-2b`，但必须先完成 P1-2a 阶段推送与远端 CI。剩余基础链按 `P1-2b → P1-3b` 严格串行。
...
| `P1-2` | P1 | 兼容实现、迁移与恢复工具 | `开发中` | `未推送` | `codex/audit-optimizations-2026-07-10`；`docs/P1-2A_IMPLEMENTATION_PLAN.md`；`docs/P1-2A_CODE_REVIEW_SUMMARY.md`；`docs/P1-2A_CODE_REVIEW_FINDINGS.md`；`docs/P1-2A_CODE_REVIEW_FINDINGS_ROUND2.md`；`docs/P1-2A_CODE_REVIEW_FINDINGS_ROUND3.md`；`docs/package-compatibility-backup-recovery-1.0.md`；ADR 0007；compatibility goldens；library maintenance engine/CLI；Doctor/Backup/Restore/Migration Plan 1.0 schemas | `P1-2a=Review 完成/未推送`：五类 MIT synthetic golden、生产 reader 零写回验证、只读 inventory/Doctor、内容寻址单论文备份、私有 journaled restore/recovery、旧 migrate 写入冻结与 dry-run 已落地；三轮 Review 无剩余阻塞项。第三轮采纳 Migration Plan 1.0 的 `not_found/invalid` 状态、authority 精确 drift 诊断、byte/mode 冲突文案、统一 fault helper、私有清理假设和 rename-race reuse 语义；planner 的 shallow `payloadsVerified:false` 按设计保留。专项 maintenance 29/29、Guard 84/84、repository/security 243/243、study 100/100 均通过，官方 validator 与 canonical reinstall 通过；active path `plugins/codex-paper/`，版本 `2.0.0+codex.20260731040626`。`P1-2b=未开始`：新 generation migration、evidence alias、reindex/drift repair 与 rollback | 创建 P1-2a 阶段提交、推送并通过远端 CI 后，再创建并实施 P1-2b | 2026-07-31 |
