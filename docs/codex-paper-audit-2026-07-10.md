...
| `P0-C1` | P0 | 防碰撞 identity、幂等与禁止静默覆盖 | `开发中` | `已推送` | `codex/audit-optimizations-2026-07-10`；commit `f947502`；`docs/P0-C1A_IMPLEMENTATION_PLAN.md`；`docs/P0-C1A_CODE_REVIEW_SUMMARY.md`；`docs/P0-C1A_CODE_REVIEW_RESULT.md`；`docs/P0-C1A_CODE_REVIEW_RESULT_ROUND2.md`；`docs/paper-identity-1.0.md`；`docs/adr/0002-paper-identity-and-generation-fingerprint.md`；Paper Identity schema/engine；generation contract；prepare/validator/skill/CI/Guard 集成 | `C1a=Review 完成`：两轮独立 Review 最终无条件 Approve 且无遗留 finding；Paper Identity 1.0、canonical/source/generation 三层身份、受信文件指纹、只读完全复用及 flat-layout fail-closed 碰撞保护已落地；在已安装 PyMuPDF 的验收环境 identity 15/15，Guard 54/54、repository/security 117/117、study/unit 74/74、PDF security 12/12、Validation 21/21、mandatory 2/2、external parser 5/5、reasoning/package 各 12/12、build/HTTP security/smoke/官方 validator 通过；[CI run 29815722103](https://github.com/byxshr/codex-paper/actions/runs/29815722103) 远端全绿，覆盖新增 Paper Identity gate 及完整工作流；已记录首次 canonical resolution 钉住与 C1b 显式 reconciliation 责任；Attention 样本得到 `arxiv:1706.03762` 且原目录 hash/mtime 不变；active 版本 `2.0.0+codex.20260721075804`。`C1b=未开始`，M2 未关闭 | 实施 `P0-C1b` 物理布局、统一 resolver 与 mutable overlay；父项继续按最保守子阶段保持开发中 | 2026-07-21 |
