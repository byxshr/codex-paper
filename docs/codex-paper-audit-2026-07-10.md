...
- 当前阶段：`M3`“必要 P1 工程与恢复能力”；M0、M1、M2 均已正式关闭，P0-C2/C2b 阶段 commit `68e53d6` 已由 [CI run 30268158984](https://github.com/byxshr/codex-paper/actions/runs/30268158984) 完整验收。P1-3a 已完成五轮独立 Review、阶段提交和推送，正在修复远端 CI 暴露的 setup-python 显式 bootstrap 接线与 hosted toolcache 权限归一化问题；Linux runtime/Docker 完整 CI 验收通过后进入 `P1-4`，后续仍按 `P1-4 → P1-2 → P1-3b` 严格串行。
...
| `P1-3` | P1 | 依赖治理、测试与仓库工程化 | `开发中` | `已推送` | `codex/audit-optimizations-2026-07-10`；P1-3a commit `3917956`；`docs/P1-3A_IMPLEMENTATION_PLAN.md`；`docs/P1-3A_CODE_REVIEW_SUMMARY.md`；五轮 `docs/P1-3A_CODE_REVIEW_FINDINGS*.md`；`docs/dependency-runtime-supply-chain-policy.md`；`security/`；runtime/dependency/secret/supply-chain scripts | `P1-3a=Review 完成`：固定 Node 22.23.1、npm 10.9.8、CPython 3.11.15、PyMuPDF 1.28.0；升级 Nuxt 4.5.1；根 audit 为 0，Web 仅保留 12 个精确、2026-08-31 到期的 high 例外；受管自包含 CPython runtime 已完成 load/rpath relocation、无 symlink且含 mode 的完整树 attestation、清洗 loader 环境后的无 bootstrap 可用性验证，parser 在使用点复核 containment，全量 repository/study suite 采用精确执行计数和失败 TAP 保留；双 digest sandbox、secret scan、policy self-validation、reviewed artifact hashes、Repository Guard 与 CI gates 已落地；五轮 Review 最终 `PASS`，无 P1-3a 遗留缺陷，不改写既有 generation/manifest。`P1-3b=未开始` | 以远端 Linux runtime/Docker CI 完成 P1-3a 交付验收；随后进入 P1-4。既有 validation 并发测试 flake 与可选诊断命名清理登记至 P1-3b | 2026-07-30 |
| `P1-4` | P1 | 统一完整 provenance | `未开始` | `未推送` | — | — | 阻塞于 P1-3a 基线；P1-3a 完成后冻结唯一 authoritative manifest schema，再启动 P1-2 | 2026-07-27 |
