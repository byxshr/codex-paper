...
Managed packages store Paper Identity at `.codex-paper/paper-identity.json` and exact generation projections in `meta.json` and `paper-data.json`. New workspaces use Identity/Generation Contract 2.0 and new publications use Generation Manifest 2.0; the 1.0 identity/manifest contracts remain read-only compatible. `paperId`, `sourceRevisionId`, and `generationId` separately represent canonical grouping, exact PDF bytes, and content-affecting generation inputs. Paper Library Layout 1.0 stores multiple generation-addressed packages beneath an identity-keyed record, resolves the default through `current.json`, and keeps tags/chat/progress/annotations in a generation-independent overlay. `index.json` is only a search/compatibility projection. Existing flat packages remain read-only until explicit migration. See [Provenance Manifest 2.0](provenance-manifest-2.0.md), [Paper Identity 1.0](paper-identity-1.0.md), and [Paper Library Layout 1.0](paper-library-layout-1.0.md).
