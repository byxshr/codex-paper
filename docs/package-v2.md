...
Managed packages add a Paper Identity 1.0 authority at `.codex-paper/paper-identity.json` and exact generation projections in `meta.json` and `paper-data.json`. `paperId`, `sourceRevisionId`, and `generationId` separately represent canonical grouping, exact PDF bytes, and content-affecting generation inputs. Paper Library Layout 1.0 stores multiple generation-addressed packages beneath an identity-keyed record, resolves the default through `current.json`, and keeps tags/chat/progress/annotations in a generation-independent overlay. `index.json` is only a search/compatibility projection. Existing flat packages remain read-only until explicit migration. P0-C2 adds workspace publication, manifest sealing, and the enforceable post-publication immutability boundary. See [Paper Identity 1.0](paper-identity-1.0.md) and [Paper Library Layout 1.0](paper-library-layout-1.0.md).
