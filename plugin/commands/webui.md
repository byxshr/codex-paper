---
allowed-tools: Bash
description: Start the Codex Paper web UI
---

## Your task

Start the Codex Paper web viewer using the production Nuxt.js server.

### Step 1: Check and Install Dependencies (First Run Only)

```bash
if [ ! -f "${CODEX_PLUGIN_ROOT}/src/web/node_modules/.package-lock.json" ] || [ ! -d "${CODEX_PLUGIN_ROOT}/src/web/node_modules/@nuxt" ]; then
  if [ ! -d "${CODEX_PLUGIN_ROOT}/src/web/node_modules/@nuxt" ]; then
    echo "ERROR: node_modules is corrupted, performing clean install..."
    cd "${CODEX_PLUGIN_ROOT}/src/web"
...
PLUGIN_VERSION=$(node -e "console.log(require('${CODEX_PLUGIN_ROOT}/.codex-plugin/plugin.json').version)")
BUILD_VERSION=""
...
if lsof -i :5815 > /dev/null 2>&1; then
  ...
  if [ -f "$PID_FILE" ] && kill -0 $(cat "$PID_FILE") 2>/dev/null; then
    ...
    if [ "$PLUGIN_VERSION" = "$BUILD_VERSION" ]; then
      echo "Codex Paper web UI is already running (version ${PLUGIN_VERSION})"
...
echo $SERVER_PID > /tmp/codex-paper-webui.pid
...
if [ $timeout -eq 0 ]; then
  ...
  rm -f /tmp/codex-paper-webui.pid
  exit 1
fi
```
...
```
Codex Paper web UI is now running!

Access it at: http://localhost:5815

To stop the server, run:
  kill $(cat /tmp/codex-paper-webui.pid)
```
