...
(async () => {
  ...
  try {
    ...
    // Fall back to PyMuPDF for PDFs that the bundled pdf-parse/pdf.js stack rejects.
    try {
...
print(json.dumps({"text": text, "numpages": len(doc)}))
`;

      const fallbackOutput = execFileSync('python3', ['-c', fallbackScript, pdfPath], {
        encoding: 'utf8'
      });

      data = JSON.parse(fallbackOutput);
    } catch (fallbackError) {
      primaryError.message = `${primaryError.message}. PyMuPDF fallback also failed: ${fallbackError.message}`;
      throw primaryError;
    }
  }
